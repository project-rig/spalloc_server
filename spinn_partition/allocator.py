"""High-level board allocation for a single SpiNNaker machine."""
...
class CandidateFilter(object):
    ...
    def _classify_links(self, boards):
        ...
        (alive, wrap, dead, dead_wrap, periphery)
            The three-tuple contains three sets of (x, y, z, link) tuples which
            enumerate links of each class in the supplied set of boards.
            
            ``alive`` links are those which are working and connect one board
            in the set to another.
            
            ``wrap`` working links between working boards in the set which
            wrap-around the toroid.
            
            ``dead`` links are those which are not working and connect one
            board in the set to another.
            
            ``dead_wrap`` dead links between working boards in the set which
            wrap-around the toroid.
            
            ``periphery`` links are those which connect from one board in the
            set to a board outside the set. These links may be dead or alive.
        """
        alive = set()
        wrap = set()
        dead = set()
...
class Allocator(object):
    ...
    def alloc_triads_possible(self, width, height, max_dead_boards=None,
        ...
        cf = CandidateFilter(self.width, self.height, self.dead_boards, self.dead_links,
                             max_dead_boards, max_dead_links, require_torus)
        for x, y in set([(0, 0),
                         (self.width - width, 0),
    ...
    def alloc_triads(self, width, height, max_dead_boards=None,
                     max_dead_links=None, require_torus=False):
        ...
        cf = CandidateFilter(self.width, self.height,
        ...
        self.next_id += 1
        self.allocation_types[allocation_id] = AllocationType.triads
        x, y = xy
    ...
    def alloc_board(self, x=None, y=None, z=None):
    ...
    def free(self, allocation_id):
        ...
        if type is AllocationType.triads:
            # Simply free the allocation
            self.pack_tree.free(x, y)
        elif type is AllocationType.board:
