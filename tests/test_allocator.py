...
class TestCandidateFilter(object):
    ...
    def test_classify_links(self):
        ...
        # For a connected block, links to internal dead boards should still be regarded
        # as peripheral. NB: The block of chips is not fully connected but
        # provides exhibits examples of all types of connectivity.
        #
        #       (1, 1, 0) ->  ___
        #                 ___/ 0 \___
    ...
    def test_alloc_triads_single(self):
        ...
        for _ in range(w * h):
            ...
            assert periphery == set((x, y, z, link)
                                    for z in range(3)
                                    for link in Links
                                    if board_down_link(x, y, z, link, w, h)[:2]
                                       != (x, y))
